...
static void s3c_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	if (s3c_nand.cpu_type == TYPE_S3C6400) {
		if (nand_type == S3C_NAND_TYPE_SLC)		nfconf &= ~S3C_NFCONF_ECC_MLC;	/* SLC */
		else						nfconf |= S3C_NFCONF_ECC_MLC;	/* MLC */
	} else {
		nfconf &= ~(0x3 << 23);

		if (nand_type == S3C_NAND_TYPE_SLC)		nfconf |= S3C_NFCONF_ECC_1BIT;
		else if(nand_type == S3C_NAND_TYPE_MLC_4BIT)	nfconf |= S3C_NFCONF_ECC_4BIT;
		else if(nand_type == S3C_NAND_TYPE_MLC_8BIT)	nfconf |= S3C_NFCONF_ECC_8BIT;
	}
	...
	if (nand_type == S3C_NAND_TYPE_MLC_4BIT || nand_type == S3C_NAND_TYPE_MLC_8BIT) {
		if (mode == NAND_ECC_WRITE)			nfcont |= S3C_NFCONT_ECC_ENC;
		else if (mode == NAND_ECC_READ)			nfcont &= ~S3C_NFCONT_ECC_ENC;
	}
...
static int s3c_nand_read_oob_1bit(struct mtd_info *mtd, struct nand_chip *chip, int page, int sndcmd)
{
	uint8_t *ecc_calc = chip->buffers->ecccalc;
	int eccbytes = chip->ecc.bytes;
	int secc_start = mtd->oobsize - eccbytes;
...
{	
	...
	for (i = 0; i < sets->nr_chips; i++)
	{
		...
		// unsigned int manuf=tmp = readb(nand->IO_ADDR_R); /* Maf. ID */
		tmp = readb(nand->IO_ADDR_R); /* Maf. ID */
		unsigned int manuf=tmp;
		...
		if (!type->pagesize) {
			if (((nand->cellinfo >> 2) & 0x3) == 0) {
				...
				if(dev_id == 0xd5) {
					...
					printk("forlinx****Nandflash:ChipType= MLC  ChipName=samsung-K9LBG08U0D*****111******* \n");
				} else if(dev_id == 0xd3) {
			...
			printk("forlinx****Nandflash:ChipType= MLC  ChipName=samsung-K9LBG08U0D******222****** \n");
